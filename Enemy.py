...
class Enemy(Entity):
    ...
    def draw(self, win, winWidth, winHeight, t):
        ...
        if self.walkCount + 1 >=18:
            self.walkCount = 0
        if self.left:
            win.blit(Enemy.walkLeft[self.walkCount//6], (self.x, self.y))
            self.walkCount+=1
        elif self.right:
            win.blit(Enemy.walkRight[self.walkCount//6], (self.x, self.y))
            self.walkCount+=1
        elif self.down:
            win.blit(Enemy.walkDown[self.walkCount//6], (self.x, self.y))
            self.walkCount+=1
