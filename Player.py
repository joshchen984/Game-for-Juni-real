...
class Player(Entity):
    ...
    def getMove(self, winWidth, winHeight, t):
        ...
        if(keys[pygame.K_a] and self.x>self.speed):
            ...
            self.up = False
            self.down = False
        elif (keys[pygame.K_d] and self.x<winWidth-self.width-self.speed):
            self.x+=self.speed * t
            self.left = False
            self.right = True
            self.up = False
            ...
            self.up = True
            self.down = False
        elif (keys[pygame.K_s] and self.y < winHeight-self.height-self.speed):
            self.y+=self.speed * t
            self.left = False
            self.right = False
            self.up = False
    ...
    def draw(self, win, winWidth, winHeight, t):
        ...
        if self.left:
            ...
            win.blit(Player.walkDown[self.walkCount//6], (self.x, self.y))
            self.walkCount+=1
        else:
            win.blit(Player.walkUp[self.walkCount//6], (self.x, self.y))
            self.walkCount+=1  
